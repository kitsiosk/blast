...
def unified_diff_newfile(string1, tofile="modified", context_lines=3):
    ...
    # Generate the unified diff
    diff = difflib.unified_diff([], lines1, fromfile=fromfile, tofile=tofile, n=context_lines)

    git_header = f"diff --git {tofile} {tofile}\nnew file mode 100644\n"
...
def slice_golden_file(golden_contents_before_arr, patch, issue_description, return_file="pre", append_line_numbers=False):
    golden_contents_after_arr, _ = apply_patch(golden_contents_before_arr, patch)
...
def get_contents_of_test_file_to_inject(row, repo_base):
...
# Read the tests generated by the SBST component to be used as seeds for the LLM component.
def get_sbst_tests(row, sbst_results_folder):
    test_dir = os.path.join(sbst_results_folder, row['instance_id'], "pynguin-tests")
